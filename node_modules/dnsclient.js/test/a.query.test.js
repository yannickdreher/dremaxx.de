import * as dnsclient from '../src/dnsclient.js';
import net from "net";

test('Query type "A" should return the correct IPv4 address', async () => {
    const question = new dnsclient.Question('dremaxx.de', dnsclient.TYPE.A, dnsclient.CLAZZ.IN);
    const result = await dnsclient.query('https://dns.dremaxx.de/dns-query', question);
    expect(result.message.flags).toHaveProperty("rcode", "NOERROR");
    expect(result.message.answers[0].data[0]).toHaveProperty("key", "ipv4");
    expect(net.isIPv4(result.message.answers[0].data[0].value)).toBe(true);
});